...
#include <stdlib.h>
#include <fstream>
#include<CommCtrl.h>              //este se usa para la funcion del DTP

#include "resource.h"
#include <iostream>

using namespace std;          //--CURP UNICA NO ESPACIOS--ENCAPSULAR FUNCIONES DE GUARDADO Y LECTURA
...
void hMenu(HWND hwnd, WPARAM wparam);
                                             //Usar solo strings para guardar todo en archivo de texto
            ...
            if (auxiliar != NULL) {
                while (salir != true) {
                    ...
                    if ((auxiliar->usuario->usuario == us) && (auxiliar->usuario->contrasena == con)) {
                        MessageBox(hwnd, L"USUARIO Y CONTRA CORRECTO", L"CORRECTO", MB_ICONEXCLAMATION);
                        CASDialog(hwnd, IDD_RP, fRP);
                    }
